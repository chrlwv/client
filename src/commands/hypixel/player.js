...
module.exports = class Player extends Command {
	...
	try {
		hypixel.getPlayerByName(this.client.hypixelKey, `${args[0]}`).then((user) => {
			if (
				!user.success ||
				user.success == false ||
				user.player == null ||
			...
			hypixel.getGuildByPlayer(this.client.hypixelKey, `${user.player.uuid}`).then((guild) => {
				...
				else tinodata.user.language = "<:charliewave_decline:771454968473190410>";
				if (
				...
				else tinodata.user.version = "<:charliewave_decline:771454968473190410>";
				if (
					guild &&
					guild.guild &&
				...
				else tinodata.user.guild = "<:charliewave_decline:771454968473190410>";
				if (
                ...
                emb = embed()
					...
					.addField('**LAST LOGIN:**',
						`${this.constructor.pad(lastLogin.getDate())}/${this.constructor.pad(
							lastLogin.getMonth() + 1
						)}/${lastLogin.getFullYear()} - ${this.constructor.pad(lastLogin.getHours())}:${this.constructor.pad(
